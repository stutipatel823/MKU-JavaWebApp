...
import com.mku.order.business.OrderService;
    ...
    public Response getOrdersByUserId(@PathParam("userId") int userId) {
        try {
            ...
            if (orders == null || orders.getOrders().isEmpty()) {
                return Response.status(Response.Status.NO_CONTENT).build(); // Return 204 if no orders found
            }

            return Response.ok(orders).build(); // Return OrdersXML as XML response if orders are found
        } catch (Exception e) {
        ...
        try {
            ...
            if (orderId == -2) {
                return Response.status(Response.Status.BAD_REQUEST)
                               .entity("Payment ID not valid or payment validation failed.")
                               .build();
            }

            // Handle order creation failure (if orderId == -1)
            if (orderId == -1) {
                return Response.status(Response.Status.BAD_REQUEST)
                               .entity("Order could not be created due to a database error.")
                               .build();
            }
